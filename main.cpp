...
struct date {
    ...
    //ввод даты последнего посещения
    void getDate() {
        printf("\nдень: "); scanf("%d", &dd);
        printf("месяц: "); scanf("%d", &mm);
        printf("год: "); scanf("%d", &yy);
    }
    //сравнение дат
    int cmpDate(date& T) {
    ...
    void showDate() { printf("            %02d.%02d.%04d           ", dd, mm, yy); }//вывод даты в консоль
};
...
struct person {
    ...
    void setPerson(char sn[], unsigned int cn, int m, date b0) {
        strcpy(Surname, sn); Card_number = cn; Money = m; last_modification = b0;
    }
...
struct table {
    ...
    void deleteTable() {
        int p;
        printf("Номер человека: "); scanf("%d", &p);
    ...
    //сумма показателя(сумма на счёте) в таблице
    int countMoney() {
        int allMoney = 0;
        for (int i = 0; i < nn; i++) {
